...
export default function Navbar() {
  ...
  const navLinks = user ? (
    <>
      {user.role === 'admin' ? (
        <Link to="/admin/dashboard" onClick={closeMobile} className={`btn-ghost ${isActive('/admin/dashboard')}`}>Admin Panel</Link>
      ) : (
        <>
          <Link to="/dashboard" onClick={closeMobile} className={`btn-ghost ${isActive('/dashboard')}`}>Dashboard</Link>
          {user.role === 'donor' && (
            <Link to="/list-food" onClick={closeMobile} className="btn-primary !py-2 !px-4 !text-sm !rounded-xl ml-1 ripple-effect hidden sm:inline-flex">
    ...
    <nav className="sticky top-0 z-50 glass">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            {user ? (
              <div className="flex items-center gap-3 ml-4 pl-4 border-l border-border">
                <Link to="/profile" className="w-8 h-8 rounded-full bg-accent/10 flex items-center justify-center text-accent text-xs font-bold shadow-sm group-hover-scale hover:ring-2 hover:ring-accent/20 transition-all">{user.name[0]}</Link>
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)} className="md:hidden p-2 rounded-xl hover:bg-accent/5 transition-colors">
            {mobileOpen ? (
          ...
          <div className="px-4 py-4 space-y-2">
            ...
            {user ? (
              <>
                ...
                <button onClick={() => { logout(); navigate('/'); closeMobile(); }} className="block w-full text-left px-4 py-3 rounded-xl text-sm font-semibold text-red-600 hover:bg-red-50 transition-all">Logout</button>
              </>
            ) : (
              <>
