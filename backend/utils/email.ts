...
export async function executeEmailSend(to: string, subject: string, html: string, text?: string) {
  const doSend = async () => {
    if (process.env.SENDGRID_API_KEY) {
      await sgMail.send({
        to,
...
export async function sendReservationApprovedEmail(to: string, userName: string, listingTitle: string, pickupAddress: string) {
...
export async function sendOrderUpdateEmail(to: string, userName: string, listingTitle: string, status: string) {
...
export async function sendOrderCancelledEmail(to: string, userName: string, listingTitle: string) {
