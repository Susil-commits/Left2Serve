...
export class PaymentService {
    static async createRazorpayOrder(amountPaise, receipt) {
    ...
    static async createOrder(userId, data) {
        ...
        if (price <= 0)
            throw new AppError(400, 'This listing is free and does not need online payment');
        ...
        const id = await insert('INSERT INTO reservations (food_listing_id, user_id, quantity, pickup_time, notes, payment_method, payment_status, amount, razorpay_order_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [food_listing_id, userId, qty, pickup_time || null, notes || null, 'razorpay', 'pending', amountRupees, order.id]);
    ...
    static async verifyPayment(userId, data) {
        ...
        if (reservation.payment_method !== 'razorpay')
            throw new AppError(400, 'This reservation is not a Razorpay payment');
        if (reservation.razorpay_order_id !== razorpay_order_id)
            throw new AppError(400, 'Order ID mismatch');
        if (reservation.payment_status === 'paid')
            return { success: true, reservation, message: 'Payment already verified' };
