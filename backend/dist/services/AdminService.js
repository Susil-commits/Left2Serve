...
export class AdminService {
    static async adminAudit(ip, action, targetType, targetId, detail) {
    ...
    static async getUsers(page, limit) {
        ...
        const users = await all('SELECT id, name, email, role, phone, address, organization, is_active, created_at FROM users ORDER BY created_at DESC LIMIT ? OFFSET ?', [limit, offset]);
    ...
    static async getListings(page, limit) {
        const offset = (page - 1) * limit;
        const listings = await all(`SELECT fl.*, u.name as donor_name, u.email as donor_email, u.phone as donor_phone, u.organization as donor_org FROM food_listings fl JOIN users u ON fl.user_id = u.id ORDER BY fl.created_at DESC LIMIT ? OFFSET ?`, [limit, offset]);
    ...
    static async deleteListing(id, ip) {
    ...
    static async updateOrder(id, status, ip) {
        ...
        if (status === 'approved')
            await createNotification(reservation.user_id, 'reservation_approved', 'Reservation approved', `Admin approved your reservation for "${title}".`, ctx);
        else if (status === 'collected')
            await createNotification(reservation.user_id, 'reservation_collected', 'Pickup completed', `Admin marked your reservation for "${title}" as collected.`, ctx);
        else if (status === 'cancelled')
            await createNotification(reservation.user_id, 'reservation_cancelled', 'Reservation cancelled', `Admin cancelled your reservation for "${title}".`, ctx);
        await this.adminAudit(ip, `order_${status}`, 'reservation', Number(id), `order #${id} -> ${status}`);
    }
    static async updateUser(id, data, ip) {
        ...
        if (data.role !== undefined) {
            ...
            if (user.role === 'admin')
                throw new Error('ADMIN_IMMUNITY');
            updates.push('role = ?');
    ...
    static async resetUserPassword(id, providedPassword, ip = '') {
    ...
    static async getTrends(days) {
        const reservations = await all(`SELECT TO_CHAR(created_at AT TIME ZONE 'UTC', 'YYYY-MM-DD') as date, COUNT(*) as count FROM reservations WHERE created_at >= NOW() AT TIME ZONE 'UTC' - INTERVAL '1 day' * ? GROUP BY 1 ORDER BY date ASC`, [days]);
        const meals = await all(`SELECT TO_CHAR(created_at AT TIME ZONE 'UTC', 'YYYY-MM-DD') as date, COALESCE(SUM(quantity), 0) as count FROM reservations WHERE status = 'collected' AND created_at >= NOW() AT TIME ZONE 'UTC' - INTERVAL '1 day' * ? GROUP BY 1 ORDER BY date ASC`, [days]);
        ...
        const byCategory = await all(`SELECT category, COUNT(*) as count FROM food_listings GROUP BY category ORDER BY count DESC`);
        return { series, byCategory };
    }
    static async getAuditLog(limit) {
        return await all('SELECT id, actor_id, actor_role, action, target_type, target_id, detail, ip, created_at FROM audit_log ORDER BY created_at DESC LIMIT ?', [limit]);
    }
    static async getExportData(type, ip) {
