...
router.post('/', authMiddleware, roleMiddleware('ngo', 'volunteer'), async (req, res) => {
    ...
    try {
        ...
        if (qty > remaining)
            return res.status(400).json({ error: `Only ${remaining} ${listing.unit} available` });
        ...
        if (existing)
            return res.status(409).json({ error: 'You already have an active reservation for this listing' });
        ...
        if (amount > 0) {
            if (payment_method === 'razorpay')
                return res.status(400).json({ error: 'Use the payment endpoint for Razorpay' });
            method = 'cod';
...
router.get('/:id/qr-token', authMiddleware, async (req, res) => {
    ...
    if (reservation.status !== 'approved')
        return res.status(400).json({ error: 'Only approved reservations can be collected' });
...
router.post('/verify-qr', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (!listing || listing.user_id !== req.user.id)
            return res.status(403).json({ error: 'Only the donor can scan this QR code' });
        if (reservation.status === 'collected')
            return res.status(400).json({ error: 'Already collected' });
        if (reservation.status !== 'approved')
            return res.status(400).json({ error: 'Reservation is not approved' });
...
router.patch('/:id', authMiddleware, async (req, res) => {
    ...
    if (!allowed.has(status))
        return res.status(400).json({ error: `Cannot set status to ${status}` });
    try {
        ...
        if (status === 'approved' && isOwner) {
            ...
            if (isOwner) {
                ...
                if (reserver?.email)
                    sendOrderUpdateEmail(reserver.email, reserver.name, title, 'collected').catch(console.error);
            }
            else if (isReserver) {
                await createNotification(donorId, 'reservation_collected', 'Pickup completed', `Your listing "${title}" was collected.`, ctx);
                if (donor?.email)
                    sendOrderUpdateEmail(donor.email, donor.name, title, 'collected').catch(console.error);
            }
        }
            ...
            if (recipient?.email)
                sendOrderCancelledEmail(recipient.email, recipient.name, title).catch(console.error);
        }
        res.json(updated);
    }
    catch (err) {
