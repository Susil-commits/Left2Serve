...
router.post('/', authMiddleware, validate(reviewSchema), async (req, res) => {
    ...
    try {
        ...
        if (!isDonor && !isReceiver)
            return res.status(403).json({ error: 'Not authorized to review this reservation' });
        if (reservation.status !== 'collected')
            return res.status(400).json({ error: 'You can only review after the pickup is completed' });
        ...
        if (existing)
            return res.status(409).json({ error: 'You have already reviewed this reservation' });
        const id = await insert('INSERT INTO reviews (reservation_id, listing_id, reviewer_id, reviewee_id, rating, comment) VALUES (?, ?, ?, ?, ?, ?)', [reservationId, listing.id, req.user.id, revieweeId, r, comment ? String(comment).trim().slice(0, 500) : null]);
...
router.get('/reservation/:id', authMiddleware, validateIdParam('id'), async (req, res) => {
    try {
        ...
        const listing = await get('SELECT id, user_id, title FROM food_listings WHERE id = ?', [reservation.food_listing_id]);
        if (!listing)
            return res.json({ canReview: false, myReview: null });
        const isDonor = listing.user_id === req.user.id;
        const isReceiver = reservation.user_id === req.user.id;
        if (!isDonor && !isReceiver)
            return res.json({ canReview: false, myReview: null });
