...
router.get('/', authMiddleware, async (req, res) => {
    try {
        const notifications = await all('SELECT id, type, title, message, data, is_read, created_at FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT 30', [req.user.id]);
...
router.get('/unread-count', authMiddleware, async (req, res) => {
    try {
        const row = await get('SELECT COUNT(*) as count FROM notifications WHERE user_id = ? AND is_read = FALSE', [req.user.id]);
