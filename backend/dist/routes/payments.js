...
router.post('/create-order', authMiddleware, roleMiddleware('ngo', 'volunteer'), async (req, res) => {
    ...
    try {
        ...
        if (price <= 0)
            return res.status(400).json({ error: 'This listing is free and does not need online payment' });
        ...
        let order;
        try {
            order = await createRazorpayOrder(amountPaise, `l2s_${req.user.id}_${Date.now()}`);
        }
        catch (e) {
            return res.status(502).json({ error: e.message || 'Failed to create payment order' });
        }
        const id = await insert('INSERT INTO reservations (food_listing_id, user_id, quantity, pickup_time, notes, payment_method, payment_status, amount, razorpay_order_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [food_listing_id, req.user.id, qty, pickup_time || null, notes || null, 'razorpay', 'pending', amountRupees, order.id]);
...
router.post('/verify', authMiddleware, roleMiddleware('ngo', 'volunteer'), async (req, res) => {
    ...
    try {
        ...
        if (reservation.payment_method !== 'razorpay')
            return res.status(400).json({ error: 'This reservation is not a Razorpay payment' });
        if (reservation.razorpay_order_id !== razorpay_order_id)
            return res.status(400).json({ error: 'Order ID mismatch' });
        if (reservation.payment_status === 'paid')
            return res.json({ success: true, reservation, message: 'Payment already verified' });
