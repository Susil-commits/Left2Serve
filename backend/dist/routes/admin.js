...
router.patch('/users/:id', authMiddleware, roleMiddleware('admin'), validateIdParam('id'), async (req, res) => {
    try {
        ...
        if (isActive === false)
            await run('UPDATE users SET token_version = token_version + 1 WHERE id = ?', [req.params.id]);
...
router.patch('/users/:id/password', authMiddleware, roleMiddleware('admin'), validateIdParam('id'), async (req, res) => {
    try {
        ...
        if (!user)
            return res.status(404).json({ error: 'User not found' });
        if (user.role === 'admin')
            return res.status(400).json({ error: 'Cannot reset an admin password' });
...
router.get('/trends', authMiddleware, roleMiddleware('admin'), async (req, res) => {
    try {
        ...
        const reservations = await all(`SELECT TO_CHAR(created_at, 'YYYY-MM-DD') as date, COUNT(*) as count FROM reservations WHERE created_at >= CURRENT_DATE - INTERVAL '1 day' * ? GROUP BY created_at::date ORDER BY date ASC`, [days]);
        const meals = await all(`SELECT TO_CHAR(created_at, 'YYYY-MM-DD') as date, COALESCE(SUM(quantity), 0) as count FROM reservations WHERE status = 'collected' AND created_at >= CURRENT_DATE - INTERVAL '1 day' * ? GROUP BY created_at::date ORDER BY date ASC`, [days]);
...
router.get('/audit-log', authMiddleware, roleMiddleware('admin'), async (req, res) => {
    try {
        ...
        const logs = await all('SELECT id, actor_id, actor_role, action, target_type, target_id, detail, ip, created_at FROM audit_log ORDER BY created_at DESC LIMIT ?', [limit]);
        res.json(logs);
    }
    catch (err) {
