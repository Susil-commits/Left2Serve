...
export class BadgeService {
   ...
   */
  static async addMealsAndCheckBadges(userId: number, quantity: number) {
    const user = await get('SELECT meals_saved, badges FROM users WHERE id = ?', [userId]);
    if (!user) return;

    const newMealsSaved = (user.meals_saved || 0) + quantity;
    const currentBadges: string[] = typeof user.badges === 'string' ? JSON.parse(user.badges) : (user.badges || []);

    const newBadgesToAward = BADGE_TIERS.filter(
      tier => newMealsSaved >= tier.threshold && !currentBadges.includes(tier.id)
    );
    ...
    for (const badge of newBadgesToAward) {
      ...
      await createNotification(
        ...
        'badge_earned',
        'New Badge Earned! 🎉',
        `Congratulations! You've saved ${newMealsSaved} meals and earned the ${badge.name} ${badge.icon} badge!`,
        { badgeId: badge.id, newMealsSaved }
      );
    }
