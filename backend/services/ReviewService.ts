...
export class ReviewService {
  static async createReview(userId: number, data: { reservationId: number, rating: number, comment?: string | null }) {
    ...
    if (!isDonor && !isReceiver) throw new AppError(403, 'Not authorized to review this reservation');
    if (reservation.status !== 'collected') throw new AppError(400, 'You can only review after the pickup is completed');
    
    const revieweeId = isDonor ? reservation.user_id : listing.user_id;
    if (revieweeId === userId) throw new AppError(400, 'You cannot review yourself');
    
    const existing = await get('SELECT id FROM reviews WHERE reservation_id = ? AND reviewer_id = ?', [reservationId, userId]);
    if (existing) throw new AppError(409, 'You have already reviewed this reservation');
    
    const id = await insert(
      'INSERT INTO reviews (reservation_id, listing_id, reviewer_id, reviewee_id, rating, comment) VALUES (?, ?, ?, ?, ?, ?)',
      [reservationId, listing.id, userId, revieweeId, r, comment ? String(comment).trim().slice(0, 500) : null]
    );
    
    return await get('SELECT * FROM reviews WHERE id = ?', [id]);
  }

  static async getReservationReviewInfo(reservationId: number, userId: number) {
    const reservation = await get('SELECT * FROM reservations WHERE id = ?', [reservationId]);
    if (!reservation) throw new AppError(404, 'Reservation not found');
    
    const listing = await get('SELECT id, user_id, title FROM food_listings WHERE id = ?', [reservation.food_listing_id]);
    if (!listing) return { canReview: false, myReview: null };
    
    const isDonor = listing.user_id === userId;
    const isReceiver = reservation.user_id === userId;
    if (!isDonor && !isReceiver) return { canReview: false, myReview: null };
  ...
  static async getUserReviews(userId: number) {
    if (!Number.isFinite(userId) || userId <= 0) return { average: 0, count: 0, reviews: [] };
    ...
    const average = count ? Math.round((reviews.reduce((a: number, r: any) => a + Number(r.rating), 0) / count) * 10) / 10 : 0;
