...
export class AuthService {
  static signToken(user: any) {
    return jwt.sign(
      { id: user.id, role: user.role, tv: Number(user.token_version || 0) },
      SECRET as string,
      { expiresIn: '7d' }
    );
  }
  ...
  static async register(payload: any) {
    ...
    const id = await insert(
      'INSERT INTO users (name, email, password_hash, role, phone, address, organization) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [String(name).trim(), normalizedEmail, password_hash, role, phone || null, address || null, organization || null]
    );
  ...
  static async login(payload: any, ip: string) {
  ...
  static async setup2FA(userId: number) {
    ...
    if (user.two_factor_enabled) throw new AppError(400, 'Two-factor authentication is already enabled');
  ...
  static async verify2FA(userId: number, token: string) {
    ...
    if (!user) throw new AppError(404, 'User not found');
    if (user.two_factor_enabled) throw new AppError(400, 'Two-factor authentication is already enabled');
    if (!user.two_factor_secret) throw new AppError(400, '2FA setup not initiated');

    const isValid = verifySync({ token, secret: user.two_factor_secret });
    if (!isValid?.valid) throw new AppError(400, 'Invalid token');
  ...
  static async disable2FA(userId: number, token: string) {
    const user = await get('SELECT two_factor_secret, two_factor_enabled FROM users WHERE id = ?', [userId]);
    if (!user) throw new AppError(404, 'User not found');
    if (!user.two_factor_enabled) throw new AppError(400, 'Two-factor authentication is not enabled');
  ...
  static async getImpact(userId: number, role: string) {
    if (role === 'donor') {
  ...
  static async updatePassword(userId: number, role: string, payload: any, ip: string) {
    const { current, newPass } = payload;
    const user = await get('SELECT password_hash, token_version FROM users WHERE id = ?', [userId]);
    
    if (!(await bcrypt.compare(current, user.password_hash))) throw new AppError(401, 'Current password is incorrect');
