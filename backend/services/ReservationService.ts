...
export class ReservationService {
  ...
  static async getListingReservations(listingId: number, userId: number) {
    const listing = await get('SELECT * FROM food_listings WHERE id = ?', [listingId]);
    if (!listing) throw new AppError(404, 'Listing not found');
    if (listing.user_id !== userId) throw new AppError(403, 'Not authorized');
  ...
  static async verifyQR(decodedToken: any, userId: number) {
    if (decodedToken.action !== 'collect') throw new AppError(400, 'Invalid token action');
    ...
    if (!listing || listing.user_id !== userId) throw new AppError(403, 'Only the donor can scan this QR code');
    if (reservation.status === 'collected') throw new AppError(400, 'Already collected');
    if (reservation.status !== 'approved') throw new AppError(400, 'Reservation is not approved');
  ...
  static async updateReservationStatus(reservationId: number, userId: number, status: string) {
    ...
    if (status === 'approved' && isOwner) {
      ...
      if (reserver?.email) sendReservationApprovedEmail(reserver.email, reserver.name, info?.title, info?.pickup_address).catch(console.error);
      ...
      if (isOwner) {
        ...
        if (reserver?.email) sendOrderUpdateEmail(reserver.email, reserver.name, info?.title, 'collected').catch(console.error);
      } else if (isReserver) {
        await createNotification(info?.user_id, 'reservation_collected', 'Pickup completed', `Your listing "${info?.title}" was collected.`, ctx);
        if (donor?.email) sendOrderUpdateEmail(donor.email, donor.name, info?.title, 'collected').catch(console.error);
      }
      ...
      if (recipient?.email) sendOrderCancelledEmail(recipient.email, recipient.name, info?.title).catch(console.error);
    }
