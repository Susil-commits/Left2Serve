...
export class AIService {
  static async generateDescription(title: string, category: string) {
    const ai = getClient();
...
Keep it under 3 sentences, professional, and empathetic. Do not include quotes around the text.`;
  ...
  static async analyzeFoodImage(imageUrl: string): Promise<{
    title: string;
    description: string;
    category: string;
  }> {
    ...
    const responseSchema: Schema = {
      ...
      properties: {
        ...
        category: {
          type: Type.STRING,
          description:
            'The most appropriate category. Must be exactly one of: event, restaurant, hotel, caterer, household',
        },
      },
