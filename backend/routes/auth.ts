...
function signToken(user: any) {
  return jwt.sign({ id: user.id, role: user.role, tv: Number(user.token_version || 0) }, SECRET as string, { expiresIn: '7d' });
}
...
router.get('/me', authMiddleware, async (req: Request, res: Response): Promise<any> => {
  const user = await get('SELECT id, name, email, role, phone, address, organization, avatar_url, created_at FROM users WHERE id = ?', [req.user!.id]);
  if (!user) return res.status(404).json({ error: 'User not found' });
  res.json(user);
});

router.get('/impact', authMiddleware, async (req: Request, res: Response): Promise<any> => {
  try {
...
router.put('/profile', authMiddleware, async (req: Request, res: Response): Promise<any> => {
  const { name, phone, address, organization, avatar_url } = req.body;
  if (name !== undefined && String(name).trim().length < 2) return res.status(400).json({ error: 'Name is too short' });
  try {
    await run('UPDATE users SET name = ?, phone = ?, address = ?, organization = ?, avatar_url = ? WHERE id = ?', [name != null ? String(name).trim() : null, phone || null, address || null, organization || null, avatar_url || null, req.user!.id]);
    const user = await get('SELECT id, name, email, role, phone, address, organization, avatar_url, created_at FROM users WHERE id = ?', [req.user!.id]);
    res.json(user);
  } catch (err) {
...
router.put('/password', authMiddleware, async (req: Request, res: Response): Promise<any> => {
