...
export async function getAvailability(listingId) {
  ...
  const [row] = await all(
    `SELECT
     ...
     FROM reservations WHERE food_listing_id = ?`,
    [listingId]
  );
...
export async function recomputeListingStatus(listingId) {
  ...
  let next;
  if (remaining > 0) next = 'available';
  else next = active > 0 ? 'reserved' : 'collected';
  if (next !== listing.status) {
    await run(
      "UPDATE food_listings SET status = ? WHERE id = ? AND status NOT IN ('expired','cancelled','collected')",
      [next, listingId]
    );
  }
}
